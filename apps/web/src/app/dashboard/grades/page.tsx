...
const mockData: AllGradesData = {
    regularGrades: [
        { designation: "Mathematics", coefficient: 3, noteCC: 14.5, noteTP: null, noteExam: 12.0 },
        { designation: "Physics", coefficient: 2, noteCC: 16.0, noteTP: 15.0, noteExam: 13.5 },
        { designation: "Computer Science", coefficient: 4, noteCC: 18.0, noteTP: 17.5, noteExam: 16.0 },
        { designation: "Electronics", coefficient: 2, noteCC: 13.0, noteTP: 14.0, noteExam: null },
        { designation: "Networks", coefficient: 3, noteCC: 15.5, noteTP: 16.0, noteExam: 14.0 },
    ],
    principalResult: {
    ...
    rattrapageGrades: [
        { designation: "Physics", coefficient: 2, noteCC: null, noteTP: null, noteExam: 11.0 },
    ],
    rattrapageResult: {
                ...
                chrome.runtime.sendMessage(
                    ...
                    (response) => {
                        if (response?.success && response.data) {
                            localStorage.setItem("esprit_grades", JSON.stringify(response.data));
    ...
    const renderContent = () => {
        ...
        switch (activeTab) {
            ...
            case "rattrapage-grades":
                const grades = activeTab === "regular"
                        ...
                        <table className="w-full">
                            <thead>
                                <tr className="border-b border-border">
                                    <th className="text-left py-4 px-4 font-semibold text-muted-foreground">Module</th>
                                    <th className="text-center py-4 px-4 font-semibold text-muted-foreground">Coef</th>
                                    <th className="text-center py-4 px-4 font-semibold text-muted-foreground">CC</th>
                                    <th className="text-center py-4 px-4 font-semibold text-muted-foreground">TP</th>
                                    <th className="text-center py-4 px-4 font-semibold text-muted-foreground">Exam</th>
                                </tr>
                            </thead>
                            <tbody>
                                {grades.map((grade, index) => (
                                    ...
                                    >
                                        <td className="py-4 px-4 font-medium">{grade.designation}</td>
                                        <td className="py-4 px-4 text-center text-muted-foreground">{grade.coefficient}</td>
                                        <td className="py-4 px-4 text-center">
                                            {grade.noteCC !== null ? (
                                                <span className={cn(
                                                    "font-semibold",
                                                    Number(grade.noteCC) >= 10 ? "text-green-500" : "text-red-500"
                                                )}>
                                                    {typeof grade.noteCC === 'number' ? grade.noteCC.toFixed(2) : grade.noteCC}
                                                </span>
                                            ) : (
                                        ...
                                        <td className="py-4 px-4 text-center">
                                            {grade.noteTP !== null ? (
                                                <span className={cn(
                                                    "font-semibold",
                                                    Number(grade.noteTP) >= 10 ? "text-green-500" : "text-red-500"
                                                )}>
                                                    {typeof grade.noteTP === 'number' ? grade.noteTP.toFixed(2) : grade.noteTP}
                                                </span>
                                            ) : (
                                                <span className="text-muted-foreground">-</span>
                                            )}
                                        ...
                                        <td className="py-4 px-4 text-center">
                                            {grade.noteExam !== null ? (
                                                <span className={cn(
                                                    "font-semibold",
                                                    Number(grade.noteExam) >= 10 ? "text-green-500" : "text-red-500"
                                                )}>
                                                    {typeof grade.noteExam === 'number' ? grade.noteExam.toFixed(2) : grade.noteExam}
                                                </span>
                                            ) : (
                                                <span className="text-muted-foreground">-</span>
                                            )}
            ...
            case "rattrapage-result":
                const result = activeTab === "principal"
                ...
                return (
                    <div className="flex flex-col items-center justify-center py-12 space-y-6">
                        <div className="text-center">
                            <p className="text-sm text-muted-foreground mb-2">General Average</p>
                ...
                {isUsingMockData && (
                    <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-3 flex items-center gap-2">
                        ...
                        <p className="text-sm text-yellow-500">
                            Showing mock data. Login with the extension to see your real grades.
                        </p>
                    </div>
                )}
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="text-left">
                                        <p className="font-semibold whitespace-nowrap">{tab.label}</p>
                                        <p className={cn(
